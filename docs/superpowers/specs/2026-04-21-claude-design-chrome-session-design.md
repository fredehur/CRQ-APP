# Claude Design — Session 1: Shared Chrome

**Date:** 2026-04-21
**Status:** Draft — pending user review
**Supersedes:** nothing. This is the first entry in a multi-session Claude Design program.
**Follow-up:** a separate spec will cover per-tab sessions (Overview, Reports, Risk Register, Context, Sources) once this session lands.

---

## Purpose

Extend the existing CRQ print design system (`docs/design/handoff/`) into a web-native design system for the AeroGrid CRQ Command Center app by running one Claude Design session scoped to shared chrome and primitive components.

Output of the Claude Design session:

- `docs/design/handoff/Command Center App.html` — golden-reference HTML showing the full chrome + a component gallery.
...
2. **Golden-reference HTML is the contract.** Same pattern that works for the three print briefs: `Command Center App.html` is a byte-equivalent target; `app.css` is lifted verbatim into the running app.
3. **Retrofit, not rebuild.** Claude Design sees the current chrome and proposes a restyle. It does not redesign information architecture.
4. **Chrome-first sequencing.** Locking chrome before per-tab sessions prevents re-negotiating button styles, spacing, and typography nine times.
...
```
bundle/
  brief.md                              # the prompt pasted into Claude Design
  current-chrome.html                   # standalone excerpt of static/index.html lines ~985–1044
  screenshots/
    01-empty.png                        # chrome with no run data
    02-mid-run.png                      # chrome with progress bar active
    03-populated.png                    # chrome with populated register bar + synthesis
  design-system/
    tokens.css                          # copied from docs/design/handoff/styles/tokens.css
    CRQ Design System.html              # copied from docs/design/handoff/
    Board Report Q2 2026.html           # copied as a working example of the design language
```

Screenshots are captured at `http://localhost:8001` via Playwright. Mid-run screenshot is captured during a real `/run-crq` invocation (mock mode acceptable).
...
- `docs/design/handoff/Command Center App.html`
...
5. Add a Playwright fidelity test: render the chrome in all three states, diff against the golden reference.

## Success criteria

- `Command Center App.html` is valid, self-contained, renders correctly in a standalone browser using only `tokens.css` + `app.css`.
...
- Every chrome state (empty, mid-run, populated) and every primitive (button, select, input, table, card, pill, chip, modal, tooltip, tab-button) appears in the golden reference.
...
3. **Gate 2 — Claude Design output approval.** After user exports from Claude Design, user reviews the output (visual fidelity to tokens, coverage of all required chrome states and primitives) before invoking the implementation plan.
