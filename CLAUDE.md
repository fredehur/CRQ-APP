...
│   └── regional-analyst-agent (Sub-agent, sonnet) — writes output/regional/{region}_draft.md
...
- Each regional analyst Task writes directly to `output/regional/{region_lowercase}_draft.md` (no shared temp file — enables parallel execution)
- Global analyst reads from `output/regional/` (all approved regional drafts)
- Global analyst writes JSON to `output/global_report.json`
- `build_dashboard.py` reads JSON + trace log → generates `output/dashboard.html` and `output/global_report.md`
...
output/
  regional/                         # Approved regional drafts ({region}_draft.md)
  .retries/                         # Circuit breaker counter files
